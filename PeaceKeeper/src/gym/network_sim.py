...
import math
from ctypes import *
currentdir = os.path.dirname(os.path.abspath(inspect.getfile(inspect.currentframe())))      #得到的是定义该函数的脚本文件的路径，包含脚本名称
parentdir = os.path.dirname(currentdir)             #去掉文件名的父目录
sys.path.insert(0,parentdir)                #定义搜索路径的优先顺序，序号从0开始，表示最大优先级
...
REWARD_SCALE = 0.001        #discount factor?
...
testlib1 = CDLL('./LEDBATPP-RL/user2.so')
class Link():
    ...
    def packet_enters_link(self, event_time):       #根据延迟确定是否丢包
...
class Network():
    ...
    def queue_initial_packets(self):
        for sender in self.senders:
            sender.register_network(self)       #注册？
            sender.reset_obs()          #重置
            heapq.heappush(self.q, (1.0 / sender.rate, sender, EVENT_TYPE_SEND, 0, 0.0, False))         # heapq.heappush() 将一个对象压入堆中
                                                                #’S‘
    def reset(self):
        self.cur_time = 0.0
        self.q = []
    ...
    def run_for_dur(self, dur,data):             #运行并计算奖励
        ...
        while self.cur_time < end_time:
            ...
            if event_type == EVENT_TYPE_ACK:                #Ack事件
                if next_hop == len(sender.path):                #path用来记录什么？
                    if dropped:
                    ...
                    push_new_event = True
            if event_type == EVENT_TYPE_SEND:           #Send事件
                if next_hop == 0:
	...
	#sender_obs
        send_ratio = sender_mi.get("send rate")
        rtt_deviation = sender_mi.get("rtt_deviation")
        rtt_gradient = sender_mi.get("recv dur")
        loss = sender_mi.get("loss ratio")
        delay = sender_mi.get("latency ratio")
        
        if send_ratio > 10:
                send_ratio = 10
        if loss > 10:
                loss = 10
        if rtt_gradient > 3000:
                rtt_gradient = 10
        ...
        # Very high thpt
        reward = (  0.2 * send_ratio  - 0.3 * loss  - 0.3 * rtt_deviation  - 0.2 * rtt_gradient)
        #reward = ( send_ratio  - 2e3 * loss - 1e2 * rtt_deviation)
        #print("Reward = %f,send_ratio = %f,loss = %f,rtt_deviation = %f,rtt_gradient = %f"% (reward,send_ratio,loss,rtt_deviation,rtt_gradient))
        




        #fo = open("rtt_deviation.txt","a")
        #fo.write("%d\n"%(rtt_deviation))
        #fo.close()
...
class Sender():
    ...
    def apply_rate_delta(self, delta):              #action-rate
    ...
    def apply_cwnd_delta(self, delta):          #action-cwnd
        delta *= config.DELTA_SCALE
        #print("Applying delta %f" % delta)
        if delta >= 0.0:
...
class SimulatedNetworkEnv(gym.Env):
    ...
    def step(self, actions):                    #RL主要流程
        ...
        self.steps_taken += 1           #步数增加
        sender_obs = self._get_all_sender_obs()         #状态观察
        sender_mi = self.senders[0].get_run_data(data)          #数据
        event = {}
        event["Name"] = "Step"
        ...
        if event["Latency"] > 0.0:
            self.run_dur = 0.5 * sender_mi.get("avg latency")       #RTT除2应该
